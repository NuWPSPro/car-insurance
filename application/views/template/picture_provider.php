...
$currPackage = $this->provider_model->current_subscription_package($uid);
...
if($parent_details['under_insititution'] == '0'){
    ...
    $ceWebPath = $mainparent_details['insititution_id'];
}
...
$citationsgetdata = $this->db
    ...
    if($this->session->userdata('logged_in')['under_insititution'] > 0){        
        ...
        $csArr = array_diff(array_column($course_certificate, 'certificate_id'),['']);
        $count_course_certificate     = count($csArr);
        }else{
            $count_course_certificate  = 0;
        }
        ...
        if($training_certificate){
            $tsArr = array_diff(array_column($training_certificate, 'certificate_id'),['']);
            $count_training_certificate = count($tsArr);
        }else{
            $count_training_certificate = 0;
        }
...
<section class="professionals-banner picprovider-banner">
    <div class="container">
        <div class="row">
            <div class="col-md-3 col-sm-4">
                ...
                <div class="prof-btns">
                  ...
                  <div class="viewprofile-editbox">
                    ...
                    <!-- <a href="javascript:void(0)" data-toggle="modal" data-target="#editbg">
                        ...
                        <a href="<?php echo site_url('provider/editCEPWebpage');?>" class="btn btn-success"><i class="fa fa-edit"></i></a>
                    </div>
                ...
                <?php } else { ?>
                    <a href="<?php echo BASE_URL.'web/'.$ceWebPath; ?>" class="btn btn-info">Institution CE Webpage</a>
    ...
    // echo $count_training_certificate;die;
    $allstaff = $count_training_certificate + $count_course_certificate;
    ?>
            ...
            <?php   }else{  
                ...
                ?>
                    ...
                    <div class="col-sm-2 col-xs-6 text-center item">
                            <div class="icon-container" style="background:#ffa500"><?php echo $count_course_certificate + $count_training_certificate;?></div>
        ...
        <?php if($this->session->userdata('logged_in')['under_insititution']>0){ 
                ...
                ?>            
                    <div class="col-md-12" style=" background: #fff; border: 1px #2d67eb solid; border-radius: 5px; height: 20px; margin-top:51px; padding:0px;">
                        <div class="traker" style="text-align:right; background:green; margin:0px;padding-right:7px;width:<?php echo $per; ?>%;height:100%;" ><?=round($persant)?> %</div>
                    </div>
            ...
            <div class="col-md-12">
                ...
                <div class="row">
                    <div class="col-xs-2 text-center">
                        <span class="advcircle"><?php echo count($totalCourse);?></span><p class="advtext">Online Courses</p>
                    </div>
                    <div class="col-xs-2 text-center">
                        <span class="advcircle" ><?php echo count($totalTraining);?></span><p class="advtext">Total Training</p>
                    </div>
                    <div class="col-xs-2 text-center">
                        <span class="advcircle"><?php echo $count_course_certificate + $count_training_certificate;?></span>
...
<div id="featurepayby" class="modal fade" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body"> 
            <a href="javascript:void(0)" onclick="payfeaturebypaypal();" class="btn"><img src="<?=base_url('assets/images/paypallogo.png') ?>" style="height:40px; width:100px;"></a>
            <a href="javascript:void(0)" onclick="payfeaturebystrip();" class="btn"><img src="<?=base_url('assets/images/OIP.jpg') ?>" style="height:40px; width:100px;"></a>
...
<div id="buyPrepaidPackage" class="modal fade" role="dialog">
    <div class="modal-dialog modal-sm text-center">
      <!-- Modal content-->
      <div class="modal-content">
        <div class="modal-header">
          <!-- <button type="button" class="close" data-dismiss="modal">&times;</button> -->
          <h4 class="modal-title">Pre-paid Digital Certificate Package</h4>
        </div>
        <div class="modal-body"> 
            <p>Go to buy Pre-paid Digital Certificate Package</p>
            <a href="<?php echo base_url('provider/pre_paid_package'); ?>" class="btn btn-primary">See Package</a>
          </div>
      </div>
    </div>
</div>
...
<form action="<?php echo base_url('stripe/index'); ?>" method="get" name="stripePay" id="fetureStripeBuyPlan">
    ...
    <input type="hidden" name="day" id="stripeDay" value="">
    <input type="hidden" name="base_price" value="<?php echo $dailprice; ?>">
    <input type="hidden" name="type" value="Promotion">
</form>
<form action="<?php echo base_url('payu/index'); ?>" method="get" name="PayuPay" id="feturepayuBuyPlan">
    <input type="hidden" name="id" value="<?php echo $userrole; ?>" > <!-- here id is role of the user. -->
    <input type="hidden" name="name" id="" value="<?php echo $user[0]['name'].' - Company Promotion';?>">
    <input type="hidden" name="price" id="totalpayuprice" value="<?php echo $pricewithtax; ?>">
    <input type="hidden" name="tax" id="payuTax" value="<?php echo $tax; ?>">
    <input type="hidden" name="day" id="payuDay" value="">
    <!--<input type="hidden" name="base_price" value="<?php echo $dailprice; ?>">-->
    <input type="hidden" name="type" value="Promotion">
</form>

<?php if($subscriptionStatus == 'n' && $this->uri->segment(2)=='set_target' && $uins == '1'){ ?> 
    <script>
        $(document).ready(function(){
            $("#buyPrepaidPackage").modal("show"); 
        });
    </script>
<?php } ?>
...
<script>
    function setprice(day){
        ...
        var totaltax = (tax*day).toFixed(2);
           jQuery('#stripestripeTax').val(tax);
           jQuery('#payuTax').val(totaltax);           
           jQuery('#stripeDay').val(day);
           jQuery('#payuDay').val(day);
        ...
        if(day){
           ...
           jQuery('#totalpayuprice').val(totalprice);
