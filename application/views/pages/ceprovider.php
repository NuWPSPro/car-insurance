...
<div class="banner">
    <div class="container">
        <div class="banner-left">  
		   <h1>ACCREDITED CAR INSURANCE COMPANIES</h1>
           <!-- <ul class="breadcrumb">
	          <li><?php if(!empty($cat[0]['cat_name'])){ echo $cat[0]['cat_name']; }else{ ?>All Professions <?php } ?></li>
	          <li><?php if($country){ echo $country[0]['countries_name']; }else{ ?>International<?php } ?></li>		
           </ul> -->
        </div>
    </div>
</div>
		...
		<div class="row displayflex">
            <div class="col-sm-8">
                <h3 class="border-title text-left">
					<?php if($flag==1){ echo'Featured ACCREDITED CAR INSURANCE COMPANIES'; }else{ echo 'Latest ACCREDITED CAR INSURANCE COMPANIES'; } ?>
				</h3>
				<div class="search-date" style="display:block">
               		...
               		<form id="courseform" class="searchform" method="GET" action="<?php echo base_url('pages/ceprovider'); ?>" >
						<div class="input-box-select">
							<input type="text" name="ceprovider_title" id="ceprovider_title" class="form-control" placeholder="NAME OF INSURANCE COMPANY" value="<?php echo $_REQUEST['ceprovider_title']; ?>">
						</div>
                ...
                <div class="row">
                <?php if ($flag == 0){ ?>
				 <?php if(count($promoted_provider))
					  {
                    ...
                    <div class="bolghome col-md-4">
						...
						<?php if(date('Y-m-d') >=$value['featured_from'] and date('Y-m-d')<=$value['featured_to']) { ?>
                            ...
                            <a href="<?php echo site_url('share/viewprofile/'.$value['id'].'');?>">
                                ...
                                <div class="training-box-caption">
                                    ...
                                    <div class="training-box_text"><?php echo $value['profession']; ?></div>
					  ...
					  <?php } 
					 }else{ echo'<p><center>No record found.</center></p>'; }   ?> 
                    <?php } ?>
                 ...
                 <?php if(count($promoted_provider))
                      {
                       foreach ($promoted_provider as $key => $value) {
                        $country = $this->db->get_where('countries',array('countries_id'=>$value['country']))->row_array()['countries_name'];
                        if($value['featured_from <=']==date('Y-m-d') && $value['featured_to >=']==date('Y-m-d')){
                        ?>              
                    <div class="bolghome col-md-4">
                        <div class="new-training-box">
                        <?php if(date('Y-m-d') >=$value['featured_from'] and date('Y-m-d')<=$value['featured_to']) { ?>
				...
				<div class="elementor-widget-button text-center">
					<a href="<?php echo base_url('users/signup/provider'); ?>" class="bg-blue">REGISTER AS INSURANCE COMPANY</a>
					<a href="<?php echo base_url('users/signup/authors');?>" class="bg-yellow">REGISTER AS INSURANCE BROKER</a>
				</div>
...
<?php 
$name = $this->db->select('name')->where(array('status'=>'1','role'=>2,'under_insititution'=>'0'))->get('tbl_user')->result_array(); 
